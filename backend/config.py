...
SQLITE_DEV_URI = f"sqlite:///{os.path.join(BASE_DIR, 'remote_control_dev.sqlite3')}"
if load_dotenv:
...
def normalize_database_url(raw_url: str | None, *, production: bool) -> str:
    """Normalize DATABASE_URL and enforce production PostgreSQL."""
    if not raw_url:
        if production:
    ...
    if production and not database_url.startswith("postgresql://"):
        raise RuntimeError("Production DATABASE_URL must be a PostgreSQL/Supabase URL.")
...
def masked_database_url(database_url: str) -> str:
    """Mask credentials in a database URL before logging."""
    parts = urlsplit(database_url)
...
class Config:
    ...
    AUTO_CREATE_MISSING_TABLES = os.getenv("AUTO_CREATE_MISSING_TABLES", "false").lower() == "true"
    SKIP_STARTUP_DB_CHECK = os.getenv("SKIP_STARTUP_DB_CHECK", "false").lower() == "true"
...
class DevelopmentConfig(Config):
    ...
    SQLALCHEMY_DATABASE_URI = normalize_database_url(
        os.getenv("DATABASE_URL") if _is_truthy("USE_DATABASE_URL_IN_DEVELOPMENT") else None,
        production=False,
    )
...
class ProductionConfig(Config):
    ...
    DEBUG = False
    ENV = "production"
    SQLALCHEMY_DATABASE_URI = normalize_database_url(os.getenv("DATABASE_URL"), production=False)
    ...
    def validate(cls) -> None:
        ...
        if not cls.SQLALCHEMY_DATABASE_URI.startswith("postgresql://"):
            raise RuntimeError("Production must use PostgreSQL. Refusing to start with SQLite.")
...
def get_config() -> type[Config]:
    ...
    if not env_name and (_is_truthy("RENDER") or os.getenv("RENDER_SERVICE_ID")):
        env_name = "production"
